...
def _get_tiktoken_encoder() -> Any:
    ...
    try:
        ...
        log.debug("tiktoken_not_installed", message="Using character-based estimation. Install tiktoken for accuracy.")
        _tiktoken_loaded = True
    ...
    def optimize_for_query(
        ...
        for memory in memories:
            ...
            if prioritize_recent and memory.get("created_at"):
                try:
                    created = datetime.fromisoformat(memory["created_at"].replace("Z", "+00:00").split("+")[0])
